...
func Strartsocks() {
	...
	for i := 0; i < len(IPliveres); i++ {
		Str := strings.Replace(IPliveres[i], "\n", "", -1)
		fmt.Println(Str)
	}
	...
	if err != nil {
		...
		if res.StatusCode == 200 {
			...
			if err == nil && strings.Contains(string(body), "当前 IP") {
				fmt.Printf("\u001B[2K\r[+]socks5: %s %s", proxyIp, string(body))
				Ip_Str := strings.Split(string(body), "来自于：")
				IPliveres = append(IPliveres, ("[+]socks5: " + proxyIp + " IP 归宿地：" + Ip_Str[1]))
				return true
			} else {
				return false
			}
...
func Readfileproxy(Coroutine int, Time int, Filename string) {
